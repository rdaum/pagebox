...
impl ThreadPlacement {
    fn build() -> Self {
        ...
        let all_performance_core_ids = match detect_performance_cores() {
        ...
        let reserve_service = match all_performance_core_ids.len() {
            0..=2 => 0,
            3..=7 => 1,
            _ => 2,
        };
...
fn thread_pinning_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        matches!(
            std::env::var("PAGEBOX_ENABLE_THREAD_PINNING")
                .ok()
                .as_deref(),
    ...
    F: FnOnce() -> T + Send + 'static,
{
    ...
    std::thread::Builder::new().name(name).spawn(move || {
        if let Some(core_id) = assigned_core_id {
            let _ = pin_current_thread_to_core(core_id);
        }
        f()
    })
}
...
fn read_physical_core_metrics() -> io::Result<Vec<PhysicalCoreMetrics>> {
    let mut physical_core_map: std::collections::HashMap<(usize, usize), PhysicalCoreMetrics> =
...
fn select_performance_cores_by_metric(
    ...
    Some(PerformanceCoreSelection {
        ...
        physical_cores: selected_cores,
        logical_processors: selected_cores,
    })
}

#[cfg(target_os = "linux")]
fn cpu_path(logical_processor_id: usize) -> std::path::PathBuf {
    std::path::Path::new(CPU_SYSFS_ROOT).join(format!("cpu{logical_processor_id}"))
}

#[cfg(target_os = "linux")]
fn read_u32(path: std::path::PathBuf) -> io::Result<u32> {
